...
namespace DispatcherfForACMESystemGenerateYearlyReport.ACMESystem
{
    ...
    internal class RegexForClientDataActivityChild : UiPath.CodedWorkflows.AsyncTaskCodedWorkflowActivity
    {
        ...
        protected override async System.Threading.Tasks.Task<Action<AsyncCodeActivityContext>> ExecuteAsync(AsyncCodeActivityContext context, System.Threading.CancellationToken cancellationToken)
        {
            ...
            var codedWorkflow = new global::DispatcherfForACMESystemGenerateYearlyReport.RegexForClientData();
            ...
            var result = await System.Threading.Tasks.Task.Run(() => CodedWorkflowHelper.RunWithExceptionHandlingAsync(() =>
            {
                ...
                {
                    codedWorkflowWithBeforeAfter.Before(new BeforeRunContext() { RelativeFilePath = "ACMESystem\\RegexForClientData.cs" });
                }
            ...
            }, (exception, outArgs) =>
            {
                if (codedWorkflow is IBeforeAfterRun codedWorkflowWithBeforeAfter)
                {
                    codedWorkflowWithBeforeAfter.After(new AfterRunContext() { RelativeFilePath = "ACMESystem\\RegexForClientData.cs", Exception = exception });
                }
